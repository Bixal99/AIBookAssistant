...
function getVapi() {
  if (!vapi) {
    if (!VAPI_API_KEY) {
      throw new Error(
        "NEXT_PUBLIC_VAPI_API_KEY is not set. Restart the dev server after updating .env or .env.local.",
      );
    }
...
const getVapiErrorMessage = (error: unknown) => {
  ...
  return "Unknown Vapi error. Check the API key, assistant ID, and network connection.";
};
...
export function useVapi(book: IBook) {
  ...
  useEffect(() => {
    const handlers = {
      ...
      error: (error: unknown) => {
        ...
        console.error("Vapi error:", error, errorMessage);
        // Don't reset isStoppingRef here - delayed events may still fire
        setStatus("idle");
        setCurrentMessage("");
        setCurrentUserMessage("");
        ...
        if (
          ...
          setLimitError(
            "Vapi is not configured correctly. Check your assistant ID and API key, then restart the dev server.",
          );
        } else {
          setLimitError(
  ...
  const start = useCallback(async () => {
    ...
    if (!ASSISTANT_ID) {
      setLimitError(
        "Vapi assistant ID is missing. Add NEXT_PUBLIC_ASSISTANT_ID to .env and restart the dev server.",
      );
      return;
    }
